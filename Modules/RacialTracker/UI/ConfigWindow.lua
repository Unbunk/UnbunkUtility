                        ...
                        build = function()
                            return {
                                -- ── Cooldown Manager integration ──────────────────────────────────────
                                {
                                    ...
                                    get = function() return ns.CDMIncludedVal(RT.CfgGet("includeInCdm")) end,
                                    set = function(v)
                                        RT.CfgSet("includeInCdm", v)
                                        RT.ApplySize()
                                        RT.ApplyPosition()
                                        RT.ApplyAll()
                                ...
                                -- ── Free icon position (only when NOT in the CDM) ─────────────────────
                                {
                                    ...
                                    onApply    = function(x, yv)
                                        if x  then RT.CfgSet("posX", x)  end
                                        if yv then RT.CfgSet("posY", yv) end
                                        RT.ApplyPosition()   -- single icon: ApplyAll alone doesn't re-anchor
                                        RT.ApplyAll()
                                    end,
                                    onUnlock   = function() RT.SetUnlocked(true)  end,
                                    onLock     = function()
                                        RT.SetUnlocked(false)
                                        if RT.pe then RT.pe.Refresh() end
                                    end,
                                    isUnlocked = function() return RT.IsUnlocked() end,
                                },

                                -- ── Icon size  W / H  (free mode only) ────────────────────────────────
                                {
                                    type   = "custom",
                                    height = 46,
                                    when   = function() return not ns.CDMIncludedVal(RT.CfgGet("includeInCdm")) end,
                                    build  = function(host)
                                        ...
                                        return {
                                            ...
                                            height  = 46,
                                            Refresh = function()
                                                wInput.SetText(tostring(RT.CfgGet("iconWidth")  or 30))
                                                hInput.SetText(tostring(RT.CfgGet("iconHeight") or 30))
                                            end,
                                        }
                                    end,
                                },
