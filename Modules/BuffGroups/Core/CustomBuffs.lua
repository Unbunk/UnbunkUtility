...
-- (we can't track arbitrary auras in combat), mirroring the reference addon Ayije_CDM's
...
-- ── Quick-Add template list (the UI's preset picker — same buffs as Ayije) ─────
...
local function CustomFrame(spellId)
    ...
    if not f then
        ...
        cd:SetReverse(true)   -- fill up as time passes (buff-style, like Ayije)
        f.Cooldown = cd
...
local function Activate(spellId, overrideStartTime)
    ...
    if f.Cooldown then
        -- C_DurationUtil drives a buff-style fill swipe (Ayije's pattern); fall back to a plain
...
-- Deactivate every running custom buff (PLAYER_DEAD; mirrors Ayije's OnPlayerDead).
