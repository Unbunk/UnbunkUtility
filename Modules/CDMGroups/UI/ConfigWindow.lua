...
local function CreatePanel(I, titleText, enableLabel, cadreTitle)
    return function(parent)
        ...
        -- The "+" tile click: add a CUSTOM (cast-triggered) cooldown. A small picker offers the Quick-Add
        -- templates (CDG.CUSTOM_COOLDOWN_TEMPLATES) as buttons plus a raw "Spell ID + Duration" pair; both
        -- route through I.AddCustom into THIS group at the given (rowIndex, colIndex) destination. Ported
        -- from BuffGroups' promptAddCustom. rowIndex == math.huge / colIndex == math.huge mean "new row" /
        -- "end of row" (I.MoveBuff / I.AddCustom resolve those).
        local CDG = ns.CDMGroups
        local function promptAddCustom(groupId, rowIndex, colIndex)
            local f = ns.ui._cdgAddCustom
            if not f then
                f = CreateFrame("Frame", "UnbunkUtilityCDGAddCustom", UIParent, "BackdropTemplate")
                f:SetSize(360, 320)
                f:SetPoint("CENTER")
                ...
                f:SetBackdropColor(0.08, 0.08, 0.08, 0.97)
                f:SetBackdropBorderColor(0.4, 0.4, 0.4, 1)
                tinsert(UISpecialFrames, "UnbunkUtilityCDGAddCustom")   -- ESC closes

                local t = f:CreateFontString(nil, "OVERLAY", "UnbunkUtilityH2")
                t:SetPoint("TOP", f, "TOP", 0, -12); t:SetText(L["Add custom"])
                ...
                f.quickButtons = {}
                local y = 64
                for _, tpl in ipairs(CDG.CUSTOM_COOLDOWN_TEMPLATES or {}) do
                    local b = ns.ui.CreateButton({ parent = f, width = 324, height = 24,
                        label = I.SpellName(tpl.spellID) })
                    b.frame:SetPoint("TOPLEFT", f, "TOPLEFT", 18, -y)
                    b.spellID = tpl.spellID
                    b.frame:SetScript("OnClick", function()
                        if f.groupId and b.spellID and I.AddCustomFromTemplate then
                            I.AddCustomFromTemplate(b.spellID, f.groupId, f.rowIndex, f.colIndex)
                        end
                ...
                local add = ns.ui.CreateButton({ parent = f, label = L["Add custom"], width = 130, height = 24 })
                ...
                add.frame:SetScript("OnClick", function()
                    ...
                    if sid and sid > 0 and f.groupId and I.AddCustom then
                        I.AddCustom(sid, f.groupId, { duration = dur or 0, rowIndex = f.rowIndex, colIndex = f.colIndex })
                    end
                    f:Hide(); if f.onDone then f.onDone() end
                end)

                ns.ui._cdgAddCustom = f
            end
            f.groupId  = groupId
            f.rowIndex = rowIndex
            f.colIndex = colIndex
