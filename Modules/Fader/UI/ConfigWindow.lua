...
local function FaderGroup(key, title)
    return {
        type = "group", title = title, build = function() return {
            ...
            -- Sub-cadre: reveal (un-fade) conditions.
            { type = "group", title = L["Reveal (show fully) when"], build = function() return {
                ...
                { type = "checkbox", label = L["Hovering it with the mouse"],
                  get = function() local c = gcfg(key); return c and c.revealMouseover end,
                  set = function(v) setField(key, "revealMouseover", v) end },
            } end },
